...
public class OggInputStream {
	...
	private boolean getPageAndPacket() {
		...
		try {
			...
			SoundSystem.logger.error("Failure reading in vorbis", e);
			endOfStream = true;
			return false;
		}
		...
		if (syncState.pageout(page) != 1) {
			...
			SoundSystem.logger.error("Input does not appear to be an Ogg bitstream.");
			endOfStream = true;
			return false;
		}
		...
		if (streamState.pagein(page) < 0) {
			...
			SoundSystem.logger.error("Error reading first page of Ogg bitstream data.");
			endOfStream = true;
			return false;
		}
		...
		if (streamState.packetout(packet) != 1) {
			...
			SoundSystem.logger.error("Error reading initial header packet.");
			endOfStream = true;
			return false;
		}
		...
		if (oggInfo.synthesis_headerin(comment, packet) < 0) {
			...
			SoundSystem.logger.error("This Ogg bitstream does not contain Vorbis audio data.");
			endOfStream = true;
			return false;
		}
		...
		while (i < 2) {
				...
				if (result == 1) {
					...
					while (i < 2) {
						...
						if (result == -1) {
							...
							SoundSystem.logger.error("Corrupt secondary header.  Exiting.");
							endOfStream = true;
							return false;
						}
			...
			try {
				...
				SoundSystem.logger.error("Failed to read Vorbis: ", e);
				endOfStream = true;
				return false;
			}
			if (bytes == 0 && i < 2) {
				SoundSystem.logger.error("End of file before finding all Vorbis headers!");
				endOfStream = true;
				return false;
			}
			syncState.wrote(bytes);
			...
			while (!endOfBitStream) {
				while (!endOfBitStream) {
					...
					if (result == -1) { // missing or corrupt data at this page
										// position
						SoundSystem.logger.error("Corrupt or missing data in bitstream; continuing...");
					} else {
				...
				if (!endOfBitStream) {
					...
					if (index >= 0) {
						...
						try {
							...
							SoundSystem.logger.error("Failure during vorbis decoding", e);
							endOfStream = true;
							return;
						}
					} else {
