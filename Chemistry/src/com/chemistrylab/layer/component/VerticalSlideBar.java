...
public class VerticalSlideBar extends Component {
	...
	public void render() {
		...
		for (int i = first < 0 ? 0 : first, count = 0; i < cons.size() && i < firstnear + canDraws - 1; i++, count++) {
			Slidable s = cons.get(i);
			s.setNowPositon(range.x0, range.x1 - barheight, MathHelper.fastFloor(mysize * count / canDraws + range.y0),
					MathHelper.fastFloor(mysize * count / canDraws + range.y0 + vertsize));
			s.render();
		}
	}
	...
	public void onMouseEvent() {
		...
		if((down=Mouse.getDWheel())!=0){
			...
			float percent = mysize / shouldDraw;
			float barlength = (percent > 1 ? 1 : percent) * mysize;
			postion = (Display.getHeight() - why - range.y0 - barlength / 2) / (mysize - barlength);
			postion = postion > 1 ? 1 : postion;
			postion = postion < 0 ? 0 : postion;
		} else {
			for (Slidable s : cons) {
